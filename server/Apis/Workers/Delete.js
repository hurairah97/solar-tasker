...
router.post("/", (req, res) => {
    ...
    const query = `
        ...
        FROM ${tableName.team} 
        WHERE member1_id = ? OR member2_id = ? OR member3_id = ?
    `;

    // Check for references in the team table
    pool.query(query, [id, id, id], (err, results) => {
        if (err) {
            logWithOptionalBroadcast('error',"Error checking ID in team table:", err);
        ...
        if (count > 0) {
            ...
            return res
                .status(400)
                .send(RESPONSE(false, "Cannot delete: ID is in use in the team table", {}));
        }
